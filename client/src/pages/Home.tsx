                    ...
                    <CardContent className="p-2">
                      <div className="space-y-0.5">
                        {prayerTimes ? prayerTimes.map((prayer) => (
                          <div key={prayer.name} className={`flex items-center justify-between px-4 py-2 rounded-2xl transition-colors group ${prayer.name.includes("Jumu'ah") ? "bg-amber-50/60 hover:bg-amber-50/100" : "hover:bg-primary/5"}`} data-testid={`prayer-${prayer.name.toLowerCase().replace(/['\s]/g, '-')}`}>
